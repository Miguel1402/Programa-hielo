...
Modelo = fem_velocidades.modelo_fem(Elementos,Velocidades,Superficie,Puntos)
x_experimental = [0.0001,0.01,0.02,0.03,0.04]
...
def graficas_errores(sensor,stagnation):
        ...
        for lwC in ['A','B','C']:
            ...
            for drop_size in [20,40,70]:
                ...
                for LWC_testing in lwc_testing: 
                    (x_int,T_sur_int)=procesado_termico_rapido(alpha,LWC_testing,T_remanso,drop_size,'intrados',V,Modelo,betha,h_c_remanso,stagnation,beta_0)
                ...
                try:
                    ...
                    #ax[fila,columna].text(predicted_LWC.x,predicted_LWC.y,f'LWC_predicted(g/$m^3$)={round(predicted_LWC.x,2)}')
                except: 
                    output.update({mvD+lwC:(LWC,np.nan)})
            # for HCT in conv_coeff:
            #     error = []
            #     for LWC_testing in lwc_testing: 
            #         (x_int,T_sur_int)=procesado_termico_rapido(alpha,LWC_testing,T_remanso,20,'intrados',V,Modelo,betha,HCT,stagnation)
            #         for i,x_intrados in enumerate(x_int):
            ...
            #     ax[fila,columna].plot(lwc_testing,error,'+-',label=f'{HCT} $cal K m^{-2}$')
            #     ax[fila,columna].plot([lwc_testing[0],lwc_testing[-1]],[Temperatura_ref,Temperatura_ref],'-r')
            #     ax[fila,columna].plot(LWC,Temperatura_ref,'or')
    ...
    return output  
Predicciones={}
for sensor in ['FBG8','FBG7','FBG6','FBG5']:
    Predicciones.update({sensor:graficas_errores(sensor,int(sensor[-1])>6)})
print(Predicciones)
plt.show()  